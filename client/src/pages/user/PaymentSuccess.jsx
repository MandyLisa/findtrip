...
const PaymentSuccess = () => {
    ...
    const handleBankTransferSuccess = async () => {
        ...
        try {
            ...
            setBookingDisplayInfo({ // data transfer object สำหรับแสดงข้อมูลการจองในหน้าผลลัพธ์
                id: bookingId,
                paymentMethod: 'โอนเงินผ่านธนาคาร',
                bankName: bankName,
                tourPackageId: booking.tourPackageId,
                title: booking.tourPackage?.title,
                startDate: booking.tourPackage?.startDate,
                endDate: booking.tourPackage?.endDate,
                adultCount: booking.adultCount,
                childCount: booking.childCount,
                singleStayCount: booking.singleStayCount,
                createdDate: booking.createdDate,
                updatedDate: booking.updatedDate,
                totalPrice: booking.totalPrice,
            })
        } catch (error) {
            console.error('Error fetching booking details for bank transfer:', error)
            setPaymentStatus('เกิดข้อผิดพลาดในการประมวลผลการชำระเงิน')
    ...
    return (
        <div className='container mx-auto p-6 bg-white rounded-lg shadow-md mt-10 max-w-full text-center'>
            ...
            {bookingDisplayInfo && ( // Hide field ถ้าไม่มีข้อมูลมาแสดง ไม่ได้ใช้ fallback || '-'
                <div className='bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6 text-left'>
                    ...
                    {bookingDisplayInfo.title && (
                        <p className='text-gray-700'><strong>ชื่อทัวร์:</strong> {bookingDisplayInfo.title}</p>
                    )}
                    {bookingDisplayInfo.startDate && (
                        <p className='text-gray-700'><strong>วันที่ออกเดินทาง:</strong> {formatThaiDate(bookingDisplayInfo.startDate)}</p>
                    )}
                    {bookingDisplayInfo.endDate && (
                        <p className='text-gray-700'><strong>วันที่สิ้นสุด:</strong> {formatThaiDate(bookingDisplayInfo.endDate)}</p>
                    )}
                    ...
                    {bookingDisplayInfo.createdDate && (
                        <p className='text-gray-700'><strong>วันที่ทำรายการ:</strong> {formatDate_Time(bookingDisplayInfo.createdDate)}</p>
                    )}
                    {bookingDisplayInfo.updatedDate && (
                        <p className='text-gray-700'><strong>วันที่ชำระเงิน:</strong> {formatDate_Time(bookingDisplayInfo.updatedDate)}</p>
                    )}
                    {bookingDisplayInfo.totalPrice && (
                        <p className='text-gray-700'><strong>ราคารวมทั้งหมด:</strong> {Number(bookingDisplayInfo.totalPrice).toLocaleString()}฿ </p>
                    )}
                </div>
            )}
            <button
